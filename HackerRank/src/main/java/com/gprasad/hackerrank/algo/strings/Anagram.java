...
public class Anagram {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < testCase; i++) {
            ...
            if (str.length() % 2 == 0) {
                System.out.println(getReplaceCharNumber(str));
            } else {
    ...
    private static int getReplaceCharNumber(String str) {
        int replaceCharNum = 0;
        ...
        for (int count : charCount) {
            if (count > 0) {
                replaceCharNum += count;
            }
        }
        return replaceCharNum;
    }
