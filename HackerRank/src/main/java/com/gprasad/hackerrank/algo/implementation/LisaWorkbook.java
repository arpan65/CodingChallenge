...
Lisa's Workbook (Hackerrank Java solution)
...
public class LisaWorkbook {
    ...
    public static void main(String[] args) {
        ...
        int noOfChapter = input.nextInt();
        int noOfProbPerPage = input.nextInt();
        int splProb = 0;
        int pageNo = 0;
        for (int p = 0; p < noOfChapter; p++) {
            int noOfProb = input.nextInt();
            int turnPage = 0;
            for (int prob = 0; prob < noOfProb; prob++) {
                if(turnPage==noOfProbPerPage)
                {
                    pageNo++;
                    turnPage=0;
                }
