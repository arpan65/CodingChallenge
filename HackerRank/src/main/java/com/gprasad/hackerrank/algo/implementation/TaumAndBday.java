...
public class TaumAndBday {
    public static void main(String[] args) {
        ...
        for(int a0 = 0; a0 < t; a0++){
            ...
            long y = in.nextLong();
            long z = in.nextLong();
            System.out.println(getTheMinSpent(b,w,x,y,z));
        }
    }

    private static long getTheMinSpent(long b, long w, long x, long y, long z) {
        long orgCostOfBlack = b*x;
        long orgCostOfWhite = w*y;
        long converCostBlack = b*(y+z);
        long conversionCostWhite = w*(x+z);
        long minCost = orgCostOfBlack+orgCostOfWhite;
        if(minCost>(orgCostOfBlack+conversionCostWhite))
        {
            minCost=orgCostOfBlack+conversionCostWhite;
        }
        if(minCost>(orgCostOfWhite+converCostBlack))
        {
            minCost = orgCostOfWhite+converCostBlack;
        }
        return minCost;
    }
}
