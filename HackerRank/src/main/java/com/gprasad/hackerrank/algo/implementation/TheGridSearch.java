/*
 The Grid Search (HackerRank java solution)
...
public class TheGridSearch {
    ...
    public static void main(String[] args) {
        ...
        for (int t = 0; t < testCase; t++) {
            int rp = in.nextInt();
            int cp = in.nextInt();
            String[] probArr = new String[rp];
            for (int i = 0; i < rp; i++) {
                probArr[i] = in.next();
            }
            int rs = in.nextInt();
            int cs = in.nextInt();
            String[] solArr = new String[rs];
            for (int i = 0; i < rs; i++) {
                solArr[i] = in.next();
            }
            System.out.println(isSolArrAvailable(rp, cp, probArr, rs, cs, solArr) ? "YES" : "NO");
        }
    }

    private static boolean isSolArrAvailable(int rp, int cp, String[] probArr, int rs, int cs, String[] solArr) {
        boolean isAvailable = false;
        for (int R = 0; R <= rp - rs; R++) {
            for (int C = 0; C <= cp - cs; C++) {
                if (probArr[R].charAt(C) == solArr[0].charAt(0)) {
                    isAvailable = doesPaternMatch(R, C, probArr, rs, cs, solArr);
                    if (isAvailable) {
                        return isAvailable;
                    }
                }
    ...
    private static boolean doesPaternMatch(int rp, int cp, String[] probArr, int rs, int cs, String[] solArr) {
        boolean doesPatternMatch = true;
        for (int r = 0; r < rs; r++) {
            for (int c = 0; c < cs; c++) {
                if (solArr[r].charAt(c) != probArr[rp + r].charAt(cp + c)) {
                    return false;
                }
            }
        }
